...
- **Envio para investigação (opcional)**: os dados do diagnóstico (sem
  identificação pessoal, já traduzidos para texto legível — ex: "Portugal"
  em vez de "pt") podem ser enviados para uma Google Sheet através de um
  Apps Script Web App. Para garantir que os dados chegam mesmo que a
  pessoa não descarregue o resumo, o envio dispara em três momentos
  possíveis (avançar do Passo 1 para o Passo 2, descarregar o resumo, ou
  terminar a sessão) — por isso a mesma sessão pode gerar mais do que uma
  linha na folha; usa-se um `sessionId` para as identificar como a mesma
  pessoa. **Este envio está desativado por omissão** — só ativa depois
  de se preencher `GOOGLE_SHEETS_URL` no topo de `js/resultados.js`. Ver
  `apps-script/Code.gs` para o código do lado do Google.
